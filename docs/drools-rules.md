...
I took the rule layering approach, when designing the rules. Layering allowed me to choose the level of control I want to exert upon the AI system.
It essentially acts as an AI pipelining system.
...
**The only rule** is that every subsequent layer after the first one, should build upon and work with the previous layer's artefacts.
